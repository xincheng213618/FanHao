...
assert(androidNativePlayerSource.match(/NativeShortVideoHttpResponse\.readUtf8\(connection, status >= 200 && status < 300\)/g)?.length === 2 && !androidNativePlayerSource.includes("readConnectionBody") && androidNativeCommentsSource.includes("NativeShortVideoHttpResponse.readUtf8(connection, status >= 200 && status < 300)") && !androidNativeCommentsSource.includes("readConnectionBody"), "Android native mutation and comment JSON must share bounded success and error response decoding");
