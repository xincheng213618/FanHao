...
function verifyWebDedicatedEntry() {
  ...
  assert(playerSource.split(/\r?\n/).length <= 8000, "the Web short-video composition root must stay below 8000 lines");
  ...
  assert(playerSource.includes("let shortVideoIconMarkupTable = null") && playerSource.includes("shortVideoIconMarkupTable || (shortVideoIconMarkupTable = {") && !playerSource.includes("const icons = {\n      ai:"), "the large SVG icon table must initialize once instead of being rebuilt for every list-card icon");
  ...
  assert(shortVideoStoreSource.includes("function encodeShortVideoListCursor") && shortVideoStoreSource.includes("function decodeShortVideoListCursor") && shortVideoStoreSource.includes("listCursor.total - listCursor.seen") && shortVideoStoreSource.includes("v.digg_count < ?"), "likes pagination must use a bounded opaque keyset cursor so concurrent metadata probes cannot shift OFFSET pages");
  ...
  assert(playerSource.includes('["following", "我的关注"]') && playerSource.includes('params.set("scope", "following")') && playerSource.includes('["authors", "following"].includes(state.shortVideo?.source)'), "My Following must reuse the author index and request only followed authors");
  ...
  assert(playerSource.includes("function renderAuthorSignature") && playerSource.includes("short-video-author-mention") && playerSource.includes("/api/short-videos/authors/resolve?mention=") && listSource.includes(".short-video-author-mention"), "web author signatures must turn @mentions into local author-page navigation");
