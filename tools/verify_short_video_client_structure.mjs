...
assert(androidEntrySource.includes("./index.js?v=20260812-collection-management-03") && androidEntrySource.includes("discardPushedView: host.navigation.discardPushedView") && androidIndexSource.includes("./collections/controller.js?v=20260812-collection-management-03"), "Android collection management and history-discard changes must propagate through the JavaScript module cache-version chain");
...
function verifyWebDedicatedEntry() {
  ...
  assert(shortVideoRuntimeSource.includes('SHORT_VIDEO_LIST_CACHE_SCHEMA = "aggregate-search-v4-native-user-actions"') && shortVideoRuntimeSource.includes("`${SHORT_VIDEO_LIST_CACHE_SCHEMA}:${url.pathname}?${normalized.toString()}`"), "short-video list response schema changes must rotate the persistent cache namespace");
