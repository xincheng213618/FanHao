...
async function verifyNativeCollectionFeedBridge() {
  ...
  const isPlayable = (video) => Boolean(String(video?.streamUrl || "").trim())
    || (String(video?.mediaType || "").toLowerCase() === "gallery"
      && (video.galleryItems || video.galleryImages || []).some((entry) => String(entry?.url || "").trim()));
  const cursorOffset = (cursor) => {
    const match = /^cursor-(\d+)$/.exec(String(cursor || ""));
    return match ? Number(match[1]) : null;
  };
  const mergeVideosById = (previous, incoming) => {
  ...
  async function verifyNativeWindow({
    ...
    nextCursor = "cursor-advanced-without-a-trusted-boundary",
    expectedCursor = ""
  }) {
    calls.length = 0;
    const selected = videos.find((item) => item.id === selectedId);
    assert(selected, `${label}: selected fixture video must exist`);
    assert.equal(await nativeFeed.openNativeShortVideoFeed(selected, {
      videos,
      hasMore: true,
      nextCursor,
      cursorBoundaries,
      feedUrl: "/api/short-videos/collections/svc_fixture/videos"
    }), true, `${label}: the actual Android Web bridge must open the collection through the Native plugin`);
    assert.equal(calls.length, 1, `${label}: the bridge must invoke the Native plugin exactly once`);
    const call = calls[0];
    const payload = JSON.parse(call.videos).videos;
    const payloadIds = payload.map(({ id }) => id);
    assert(payloadIds.length > 0 && payloadIds.length <= 51, `${label}: the Native bridge window must contain at most 51 playable entries`);
    assert(payloadIds.includes(selectedId), `${label}: the Native payload must contain startId`);
    assert(call.startIndex >= 0 && call.startIndex < payloadIds.length, `${label}: startIndex must stay inside the Native payload`);
    assert.equal(payloadIds[call.startIndex], selectedId, `${label}: startIndex must resolve to startId`);
    assert.equal(call.nextCursor, expectedCursor, `${label}: only a cursor matching the emitted slice boundary may be forwarded`);

    const continuationOffset = cursorOffset(expectedCursor) ?? call.nextOffset;
    assert(Number.isInteger(continuationOffset) && continuationOffset >= 0, `${label}: fallback offset must match a concrete emitted slice boundary`);
    const continuationIds = fullUniverse.slice(continuationOffset).filter(isPlayable).map(({ id }) => id);
    const traversed = payloadIds.concat(continuationIds);
    const expectedStart = fullUniverse.findIndex(({ id }) => id === payloadIds[0]);
    assert(expectedStart >= 0, `${label}: emitted payload must belong to the collection universe`);
    const expected = fullUniverse.slice(expectedStart).filter(isPlayable).map(({ id }) => id);
    assert.deepEqual(traversed, expected, `${label}: Native next traversal must have no gaps`);
    assert.equal(new Set(traversed).size, traversed.length, `${label}: Native next traversal must have no duplicates`);
    return { call, payloadIds };
  }

  try {
    for (const selectedIndex of [5, 70, 140]) {
      const { payloadIds, call } = await verifyNativeWindow({
        label: `trusted boundary at collection index ${selectedIndex}`,
        videos: loaded,
        selectedId: loaded[selectedIndex].id,
        cursorBoundaries: boundaries,
        nextCursor: "cursor-144",
        expectedCursor: selectedIndex === 5 ? "" : selectedIndex < 96 ? "cursor-96" : "cursor-144"
      });
    ...
    const withoutFirstBoundaryUniverse = universe.filter(({ id }) => id !== loaded[47].id);
    const withoutFirstBoundary = loaded.filter(({ id }) => id !== loaded[47].id);
    await verifyNativeWindow({
      label: "deleted current boundary with a distant next boundary",
      videos: withoutFirstBoundary,
      selectedId: loaded[20].id,
      cursorBoundaries: removeCollectionCursorBoundaryVideo(boundaries, loaded[47].id),
      fullUniverse: withoutFirstBoundaryUniverse
    });

    const withoutPreviousAndCurrentIds = new Set([loaded[47].id, loaded[95].id]);
    const withoutPreviousAndCurrentUniverse = universe.filter(({ id }) => !withoutPreviousAndCurrentIds.has(id));
    const withoutPreviousAndCurrent = loaded.filter(({ id }) => !withoutPreviousAndCurrentIds.has(id));
    await verifyNativeWindow({
      label: "deleted previous and current boundaries",
      videos: withoutPreviousAndCurrent,
      selectedId: loaded[70].id,
      cursorBoundaries: [...withoutPreviousAndCurrentIds].reduce(removeCollectionCursorBoundaryVideo, boundaries),
      fullUniverse: withoutPreviousAndCurrentUniverse
    });

    const withoutNextBoundaryUniverse = universe.filter(({ id }) => id !== loaded[143].id);
    const withoutNextBoundary = loaded.filter(({ id }) => id !== loaded[143].id);
    await verifyNativeWindow({
      label: "deleted next boundary at the loaded tail",
      videos: withoutNextBoundary,
      selectedId: loaded[138].id,
      cursorBoundaries: removeCollectionCursorBoundaryVideo(boundaries, loaded[143].id),
      fullUniverse: withoutNextBoundaryUniverse,
      nextCursor: "cursor-144"
    });

    await verifyNativeWindow({
      label: "next trusted boundary more than 30 playable items ahead",
      videos: loaded,
      selectedId: loaded[60].id,
      cursorBoundaries: boundaries.filter(({ endVideoId }) => endVideoId === loaded[95].id),
      nextCursor: "cursor-144"
    });

    const partialDuplicatePage = [loaded[47], ...loaded.slice(48, 95)];
    const partialDuplicateVideos = mergeVideosById(loaded.slice(0, 48), partialDuplicatePage);
    const partialDuplicateBoundaries = appendCollectionCursorBoundary(boundaries.slice(0, 1), {
      videos: partialDuplicatePage,
      hasMore: true,
      nextCursor: "cursor-95"
    }, partialDuplicateVideos, loaded.slice(0, 48));
    assert.equal(partialDuplicateBoundaries.length, 1, "a partially duplicate page must not create an ambiguous trusted boundary");
    await verifyNativeWindow({
      label: "partially duplicate page with controller cursor advanced",
      videos: partialDuplicateVideos,
      selectedId: loaded[80].id,
      cursorBoundaries: partialDuplicateBoundaries,
      nextCursor: "cursor-95"
    });

    const fullyDuplicateVideos = loaded.slice(0, 48);
    const fullyDuplicateBoundaries = appendCollectionCursorBoundary(boundaries.slice(0, 1), {
      videos: fullyDuplicateVideos,
      hasMore: true,
      nextCursor: "cursor-duplicate-page"
    }, fullyDuplicateVideos, fullyDuplicateVideos);
    assert.equal(fullyDuplicateBoundaries.length, 1, "a fully duplicate page must not advance the trusted boundary");
    await verifyNativeWindow({
      label: "fully duplicate page with controller cursor advanced",
      videos: fullyDuplicateVideos,
      selectedId: loaded[40].id,
      cursorBoundaries: fullyDuplicateBoundaries,
      nextCursor: "cursor-duplicate-page",
    ...
    const readdedDuplicatePage = [readdedFirstPage[0], ...readdedUniverse.slice(48, 95)];
    const readdedVideos = mergeVideosById(readdedFirstPage, readdedDuplicatePage);
    let readdedBoundaries = appendCollectionCursorBoundary([], {
      videos: readdedFirstPage,
      hasMore: true,
      nextCursor: "cursor-48"
    }, readdedFirstPage, []);
    readdedBoundaries = appendCollectionCursorBoundary(readdedBoundaries, {
      videos: readdedDuplicatePage,
      hasMore: true,
      nextCursor: "cursor-95-after-readd"
    }, readdedVideos, readdedFirstPage);
    await verifyNativeWindow({
      label: "concurrent remove and re-add with a duplicate page",
      videos: readdedVideos,
      selectedId: readdedUniverse[80].id,
      cursorBoundaries: readdedBoundaries,
      fullUniverse: readdedUniverse,
      nextCursor: "cursor-95-after-readd"
    });

    const mixedUniverse = universe.map((item, index) => index % 7 === 0 ? { ...item, streamUrl: "" } : item);
    const mixedLoaded = mixedUniverse.slice(0, 144);
    let mixedBoundaries = [];
    for (let offset = 0; offset < mixedLoaded.length; offset += 48) {
      mixedBoundaries = appendCollectionCursorBoundary(mixedBoundaries, {
        videos: mixedLoaded.slice(offset, offset + 48),
        hasMore: true,
        nextCursor: `cursor-${offset + 48}`
      }, mixedLoaded.slice(0, offset + 48), mixedLoaded.slice(0, offset));
    }
    const mixedWithoutBoundaryUniverse = mixedUniverse.filter(({ id }) => id !== mixedLoaded[47].id);
    const mixedWithoutBoundary = mixedLoaded.filter(({ id }) => id !== mixedLoaded[47].id);
    await verifyNativeWindow({
      label: "deleted boundary with unplayable entries mixed into pages",
      videos: mixedWithoutBoundary,
      selectedId: mixedLoaded[20].id,
      cursorBoundaries: removeCollectionCursorBoundaryVideo(mixedBoundaries, mixedLoaded[47].id),
      fullUniverse: mixedWithoutBoundaryUniverse
    });
...
function verifyWebShortVideoRouter() {
  ...
  const sizeSorted = webShortVideoRouteFromUrl("http://127.0.0.1/short-videos?source=all&sort=size");
  assert.equal(sizeSorted.shortVideoSort, "size", "the dedicated web router must preserve file-size sorting");
  assert.equal(webShortVideoRouteUrl(sizeSorted, { initialParams: new URLSearchParams(), hash: "" }), "/short-videos?source=all&sort=size");
...
const androidCardActionMetricSource = readNormalized(path.join(moduleDir, "list", "card-action-metric.js"));
...
const androidNativeDeleteControllerSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoDeleteController.java"));
const androidNativeDeleteTransportSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoDeleteTransport.java"));
const androidNativeDeleteJobStateSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoDeleteJobState.java"));
const androidNativeDeleteStatusViewSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoDeleteStatusView.java"));
...
const androidNativeFeedAutoAdvanceSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoFeedAutoAdvance.java"));
...
const androidNativeHttpResponseSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoHttpResponse.java"));
...
const androidNativeActionResultDecoderSource = readNormalized(path.join(root, "android-client", "android", "app", "src", "main", "java", "local", "fanhao", "library", "NativeShortVideoActionResultDecoder.java"));
...
assert(androidNativeFeedSource.includes("nextCollectionCursorBoundary") && androidNativeFeedSource.includes("nextCursor: cursorAtEnd") && androidNativeFeedSource.includes('const cursorAtEnd = boundary?.nextCursor || ""') && androidNativeFeedSource.includes("playableEnd - selectedIndex > 31") && androidPlayerPluginSource.includes("EXTRA_NEXT_CURSOR") && androidNativeFeedModelsSource.includes('String nextCursor = ""'), "Android native collection playback must align its bounded WebView window with the exact opaque continuation boundary or its emitted offset fallback");
...
assert(androidNativeHttpResponseSource.includes("return new String(readBytes(input, maxBytes), StandardCharsets.UTF_8)") && !androidNativePlayerSource.includes("new String(buffer") && !androidNativeCommentsSource.includes("new String(buffer") && androidNativeFeedTransportSource.includes("NativeShortVideoHttpResponse.readUtf8(connection, true)"), "Android native JSON responses must decode UTF-8 only after bounded byte collection");
assert(androidNativePlayerSource.match(/NativeShortVideoHttpResponse\.readUtf8\(connection, status >= 200 && status < 300\)/g)?.length === 1 && androidNativeDeleteTransportSource.includes("NativeShortVideoHttpResponse.readUtf8(connection, status >= 200 && status < 300)") && !androidNativePlayerSource.includes("readConnectionBody") && !androidNativeDeleteTransportSource.includes("readConnectionBody") && androidNativeCommentsSource.includes("NativeShortVideoHttpResponse.readUtf8(connection, status >= 200 && status < 300)") && !androidNativeCommentsSource.includes("readConnectionBody"), "Android native mutation and comment JSON must share bounded success and error response decoding across their controller transports");
assert(androidNativeHttpResponseSource.includes("rejectDeclaredOverflow(connection, maxBytes)") && androidNativeHttpResponseSource.includes("read > maxBytes - total") && androidNativeImageLoaderSource.includes("status < 200 || status >= 300") && androidNativeImageLoaderSource.includes("startsWith(\"image/\")") && androidNativeImageLoaderSource.includes("MAX_IMAGE_BYTES = 8 * 1024 * 1024") && androidNativePlayerSource.includes("NativeShortVideoImageLoader.read(connection)"), "Android native image loads must enforce HTTP, media type, declared-length, and streaming size limits");
assert(androidNativePlayerSource.includes("NativeShortVideoFeedAutoAdvance.resolve(") && androidNativePlayerSource.includes("Action.LOAD_MORE") && androidNativePlayerSource.includes("Action.END") && androidNativePlayerSource.includes('showStatus("暂无短视频")') && !androidNativePlayerSource.includes('showStatus("短视频读取失败")'), "Android native paging must retry duplicate pages, finish duplicate final pages, and keep a successful empty feed distinct from failure");
assert(androidNativeFeedAutoAdvanceSource.includes("paging.clearPendingAutoAdvance()") && androidNativeFeedAutoAdvanceSource.includes("return Action.LOAD_MORE") && androidNativeFeedAutoAdvanceSource.includes("return Action.END"), "Android duplicate-page auto-advance state must have one testable owner");
...
assert(androidPlayerPluginSource.includes("NativeShortVideoActionResultDecoder.decode(") && androidNativeActionResultDecoderSource.includes("MAX_SNAPSHOTS = 512") && androidNativeActionResultDecoderSource.includes("MAX_VIDEO_ID_CHARS = 512") && androidNativeActionResultDecoderSource.includes("rawJson.length() > MAX_JSON_CHARS") && androidNativeActionResultDecoderSource.includes("source.length() > MAX_SNAPSHOTS"), "the Java Activity-result bridge must reject oversized payloads, oversized snapshot arrays, and oversized video IDs before crossing into JavaScript");
...
assert(androidListControllerSource.includes("primaryResult.revalidation.then") && androidListControllerSource.indexOf("primaryResult.revalidation.then") < androidListControllerSource.indexOf("if (!isCurrentRequest()) return;\n      applyLoadedListData") && androidListControllerSource.includes("listEnvelopeSignature") && androidListControllerSource.includes("nextVideos.map(revalidationVideoId)"), "Android cached-list revalidation must install rejection handling before stale guards and fully render fresh order or envelope changes");
...
assert(androidNativeFeedContractSource.includes("liked: Boolean(item.actions?.liked)") && androidNativeFeedContractSource.includes("collected: Boolean(item.actions?.collected)"), "Android native feed bridge must preserve both server-backed action states");
assert(androidNativeFeedContractJavaSource.includes('actions.optBoolean("liked", false)') && androidNativeFeedContractJavaSource.includes('actions.optBoolean("collected", false)') && androidNativeFeedModelsSource.includes("boolean libraryLiked;") && androidNativeFeedModelsSource.includes("boolean userCollected;"), "Android native models must keep downloaded-library likes distinct from the two server-backed user actions");
assert(androidNativePlayerSource.includes("NativeShortVideoActionState") && androidNativePlayerSource.includes('payload.put("active", mutation.active)') && androidNativePlayerSource.includes("completeFailure(mutation)") && !androidNativePlayerSource.includes("item.likes + (!item.libraryLiked"), "Android action mutations must send explicit active values and roll back through the generation state machine without double-counting metrics");
...
assert(androidNativePlayerSource.includes("playbackFallback.handle(player, videos.get(index), error") && androidNativePlaybackFallbackSource.includes('SMOOTH_RENDITION = "mobile-2560-h264-v3"') && androidNativePlaybackFallbackSource.includes("/media/short-video-smooth/") && androidNativePlaybackFallbackSource.includes("reportIssue(item, error)"), "Android short-video HEVC decode failures must switch once to the bounded H.264 smooth rendition and report the observed issue for caching");
assert(androidNativePlayerSource.includes("cachedMediaUri(playbackFallback.playbackUrl(item))") && androidNativePlayerSource.includes("playbackFallback.mediaUri(player, item)") && androidNativePlaybackFallbackSource.includes("fallbackIds.contains(item.id)"), "Android short-video retries must keep using the compatibility rendition instead of looping on the unsupported source URL");
assert(androidNativePlaybackFallbackSource.includes("scheduleRetry(player, item, shouldPlay)") && androidNativePlaybackFallbackSource.includes("AUTO_RETRIES = 4") && webPlaybackRenditionPolicySource.includes("function retrySmoothFallback") && webPlaybackRenditionPolicySource.includes("shortVideoSmoothRetryCount") && webPlaybackRenditionPolicySource.includes("smooth-rendition-retry-${attempt}"), "web and Android short-video players must automatically retry the smooth URL while its first observed HEVC rendition finishes generating");
...
function verifyWebDedicatedEntry() {
  ...
  const likeDistributionCacheSource = readNormalized(path.join(root, "public", "modules", "short-videos", "like-distribution-cache.js"));
  const likeDistributionLoaderSource = readNormalized(path.join(root, "public", "modules", "short-videos", "like-distribution-loader.js"));
  const likeDistributionPageShellSource = readNormalized(path.join(root, "public", "modules", "short-videos", "like-distribution-page-shell.js"));
  ...
  const localActionsSource = readNormalized(path.join(root, "public", "modules", "short-videos", "local-actions.js"));
  ...
  const authorCleanupSource = readNormalized(path.join(root, "public", "modules", "short-videos", "author-cleanup.js"));
  const authorProfileHistorySource = readNormalized(path.join(root, "public", "modules", "short-videos", "author-profile-history.js"));
  ...
  const shortVideoSmoothWarmupWorkerClientSource = readNormalized(path.join(root, "src", "modules", "short-videos", "server", "smooth-warmup-worker-client.js"));
  const shortVideoSmoothWarmupWorkerSource = readNormalized(path.join(root, "src", "modules", "short-videos", "server", "smooth-warmup-worker.js"));
  ...
  assert(collectionsControllerSource.includes('event.key === "Escape"') && collectionsControllerSource.includes('event.key !== "Tab"') && collectionsControllerSource.includes('setAttribute("aria-current", "page")'), "Web collection picker and sidebar must retain modal keyboard and current-page accessibility semantics");
  assert(!playerSource.includes('document.createElement("main")') && !collectionsControllerSource.includes('document.createElement("main")'), "Web short-video views must not nest main landmarks inside the shared application main");
  ...
  assert(entrySource.includes("ensureShortVideoViewerStyles: () => window.__fanhaoEnsureShortVideoViewerStyles") && playerSource.includes("const viewerStylesReady = Promise.resolve(ensureShortVideoViewerStyles?.())") && playerSource.includes("const detailPromise = options.detailData") && playerSource.includes("contextualShortVideoDetail(videoId)") && playerSource.includes("Promise.all([detailPromise, viewerStylesReady])"), "opening a video from the list or membership detail must overlap media/detail work with deferred viewer stylesheet loading before rendering the player");
  ...
  assert(!shortVideoRuntimeSource.includes("copyFileSync") && !shortVideoRuntimeSource.includes("fs.promises.copyFile") && shortVideoRuntimeSource.includes("copyVideoStartupToSharedCache") && shortVideoRuntimeSource.includes("const videoCacheQueue = []"), "short-video playback may cache only the bounded startup prefix, never a hidden full-video copy");
  ...
  assert(shortVideoRuntimeSource.includes('"X-FanHao-Media-Cache": file.cachedStartup ? "startup" : "source"'), "short-video ranges must identify only startup-prefix and source paths for live verification");
  ...
  assert(shortVideoRuntimeSource.includes("SHORT_VIDEO_SMOOTH_CACHE_BACKLOG_LIMIT = 512") && shortVideoRuntimeSource.includes("SHORT_VIDEO_SMOOTH_CACHE_PAGE_SIZE = 256") && shortVideoRuntimeSource.includes("SHORT_VIDEO_SMOOTH_CACHE_REFILL_LOW_WATERMARK = 256") && shortVideoRuntimeSource.includes("smoothWarmupWorker.queryCount()") && shortVideoRuntimeSource.includes("smoothWarmupWorker.queryCandidates(pageLimit, smoothVideoCandidateScanOffset)") && shortVideoSmoothWarmupWorkerSource.includes("catalogStore.smoothPlaybackCandidateCount()") && shortVideoSmoothWarmupWorkerSource.includes("catalogStore.smoothPlaybackCandidates(message.limit, message.offset)") && shortVideoSmoothWarmupWorkerClientSource.includes('return request("candidates", { limit, offset })') && shortVideoStoreSource.includes("function reportSmoothPlaybackIssue") && shortVideoStoreSource.includes("CROSS JOIN short_videos video ON video.id = issue.video_id") && shortVideoStoreSource.includes("issue.active = 1") && shortVideoRuntimeSource.includes("smoothVideoCandidateBacklog") && shortVideoRuntimeSource.includes("refillSmoothVideoCandidateBacklog()"), "only videos with an observed playback issue must page through the bounded worker-backed background backlog");
  ...
  assert(shortVideoRuntimeSource.includes("autoSmoothWarmup = false") && shortVideoRuntimeSource.includes("if (autoSmoothWarmup) schedule4kSmoothVideoWarmup();") && shortVideoRuntimeSource.includes("if (autoSmoothWarmup) schedule4kSmoothVideoWarmup(250);") && shortVideoStoreSource.includes("ORDER BY issue.last_reported_at DESC"), "background rendition warmup must be opt-in while explicit playback issues remain ordered by recency");
  assert(shortVideoRuntimeSource.includes("async function refreshSmoothVideoCacheEntryIndex") && shortVideoRuntimeSource.includes("await fs.promises.readdir(cacheDir, { withFileTypes: true })") && shortVideoRuntimeSource.includes("rememberIndexedSmoothVideoCandidate(video)") && shortVideoStoreSource.includes("sourcePath: String(row.source_path || \"\")") && shortVideoRuntimeSource.includes("warmupDurationMs"), "smooth-rendition warmup must asynchronously index the cache directory once instead of statting hundreds of resolved candidates on the HTTP thread");
  ...
  assert(shortVideoRuntimeSource.includes('req.headers?.["x-fanhao-client"]') && shortVideoRuntimeSource.includes("!androidPlayback && !hasIfRange(req)") && shortVideoRuntimeSource.includes("file.maxRangeBytes = androidPlayback") && shortVideoRuntimeSource.includes("file.fullResponse = androidPlayback"), "Android Media3 requests must bypass physical startup-prefix files and artificial open-ended range truncation");
  ...
  assert(!shortVideoRuntimeSource.includes("queueWatchedVideoCache") && !shortVideoRuntimeSource.includes("SHORT_VIDEO_CACHE_MIN_PROGRESS_MS") && shortVideoRoutesSource.includes("onWatch?.(videoId, body || {}, data)"), "watch progress must not trigger a hidden full-video cache copy");
  ...
  assert(shortVideoRuntimeSource.includes("const routeStore = new Proxy(store")
    ...
    && shortVideoRuntimeSource.includes("sqliteRuntimeStamp(downloadManagerDbPath)")
    && shortVideoRuntimeSource.includes('fileRuntimeStamp(`${target}-wal`, { ignoreEmpty: true })')
    && shortVideoRuntimeSource.includes("catalogWorker.invalidateLikeDistribution()")
    ...
    && shortVideoCatalogWorkerClientSource.includes("likeDistributionGeneration !== flightGeneration")
    && shortVideoCatalogWorkerClientSource.includes("invalidateLikeDistribution")
  ...
  assert(shortVideoRuntimeSource.includes("skipStartupMaintenance: true") && shortVideoRuntimeSource.includes("store.prepareSchema();") && shortVideoRuntimeSource.includes("initialStateKey: () => store.downloadManagerSourceStateKey()") && !shortVideoRuntimeSource.includes("store.warm();"), "the HTTP runtime must run only the required writable schema preflight and must not prewarm the multi-gigabyte catalog or stats worker");
  ...
  assert(shortVideoRuntimeSource.includes('SHORT_VIDEO_LIST_CACHE_SCHEMA = "aggregate-search-v5-action-baselines"') && shortVideoRuntimeSource.includes("`${SHORT_VIDEO_LIST_CACHE_SCHEMA}:${url.pathname}?${normalized.toString()}`"), "short-video list response schema changes must rotate the persistent cache namespace");
  ...
  assert((shortVideoRoutesSource.match(/onMutation\?\.\(\);/g) || []).length >= 6 && shortVideoRoutesSource.includes("? await recordWatch(videoId, body || {})") && shortVideoRoutesSource.includes("onWatchMutation?.(videoId, body || {}, data);") && shortVideoRuntimeSource.includes("watchWriter.record(videoId, options)") && shortVideoWatchWriterSource.includes('workerUrl = new URL("./watch-write-worker.js"') && shortVideoWatchWorkerSource.includes("store.recordWatch(message.videoId"), "catalog mutations must invalidate all list generations while high-frequency watch writes use their dedicated worker path");
  ...
  assert((playerSource.match(/refreshShortVideoDeleteSelectionUi\(\);/g) || []).length === 4 && listCardsSource.includes("function syncRenderedSelection(root)") && listCardsSource.includes('card.classList.toggle("is-selected", selected)') && listCardsSource.includes('open.setAttribute("aria-pressed", String(selected))'), "Web delete selection must patch the toolbar and mounted cards in place so selecting a deep virtual-list item cannot reset the scroll position");
  ...
  assert(playerSource.includes("createShortVideoLocalActions({ api, showToast: showBrowserToast })") && localActionsSource.includes('new Set(["reveal", "open-author-folder"])') && localActionsSource.includes('`/media/short-video/${encodeURIComponent(videoId)}?download=1`') && localActionsSource.includes('`/api/short-videos/${encodeURIComponent(videoId)}/local-action`') && localActionsSource.includes("body: { action: normalizedAction }"), "Web local-file actions must use server-resolved video IDs for reveal/author-folder operations and the attachment media contract for downloads");
  assert(playbackSettingsSource.includes('localTitle.textContent = "本地文件"') && playbackSettingsSource.includes('"下载原文件"') && playbackSettingsSource.includes('runShortVideoLocalAction(video, "reveal")') && playbackSettingsSource.includes('runShortVideoLocalAction(video, "open-author-folder")') && webIconsSource.includes("download:") && webIconsSource.includes("folder:"), "playback settings must expose download, source-file reveal, and author-folder operations in a dedicated local-files section");
  assert(authorPanelSource.includes('authorFolder.textContent = "作者文件夹"') && authorPanelSource.includes('runShortVideoLocalAction(video, "open-author-folder", authorFolder)') && authorPagesSource.includes('runShortVideoLocalAction(authorVideo, "open-author-folder", authorFolder)') && authorPagesSource.includes("authorFolder.disabled = !authorVideo?.id"), "author sidebars and standalone author pages must open the author folder through a representative local video ID");
  assert(playerSource.includes('playback-settings.js?v=20260824-local-file-actions-01') && playerSource.includes('author-panel.js?v=20260824-local-file-actions-01') && playerSource.includes('els.workGrid?.querySelector?.(".short-video-more-overlay")'), "local-action lazy modules must use fresh cache versions and an open more-menu must suppress continuous-play navigation");
  ...
  assert(authorPagesSource.includes("renderShortVideoAuthorProfileHistory") && authorProfileHistorySource.includes('"曾用名"') && authorProfileHistorySource.includes('"获赞记录"') && listSource.includes(".short-video-author-page-history-chip"), "author detail must expose persisted previous names and observed total-like history");
  ...
  assert(likeDistributionLoaderSource.includes('"/api/short-videos/like-distribution/refresh"') && likeDistributionLoaderSource.includes('forceRefresh ? { method: "POST" } : {}') && likeDistributionSource.includes('loadLikeDistribution({ force: true })') && likeDistributionSource.includes('refresh.textContent = state.shortVideo.likeDistributionLoading ? "重新统计中…" : "手动刷新"') && likeDistributionSource.includes("缓存生成") && shortVideoRoutesSource.includes('url.pathname === "/api/short-videos/like-distribution/refresh"'), "statistics must use cached GET data by default while the explicit manual action forces an administrator-only rebuild");
  assert(likeDistributionLoaderSource.includes('if (options.force === true) {') && likeDistributionLoaderSource.includes('const shouldRender = !shortVideoState.likeDistributionLoading && shortVideoState.mode === "likes";') && likeDistributionLoaderSource.includes('if (shouldRender) render();'), "manual statistics refresh must visibly promote an in-flight silent refresh instead of appearing unresponsive");
  assert(playerSource.includes('classList.remove("app-module-loading")') && likeDistributionPageShellSource.includes("正在准备内容洞察") && likeDistributionSource.includes("正在读取统计缓存") && likeDistributionSource.includes("完成前继续显示上一次结果") && listSource.includes(".short-video-distribution-progress-track") && listSource.includes("short-video-distribution-spin"), "slow statistics startup and refreshes must reveal an accessible visible progress state instead of leaving the direct route blank");
  assert(likeDistributionCacheSource.includes("CACHE_FRESH_MS") && likeDistributionCacheSource.includes("readLikeDistributionCache") && likeDistributionCacheSource.includes("writeLikeDistributionCache") && playerSource.includes("silent: true"), "repeat statistics visits must reuse a fresh local response and only revalidate expired cache in the background");
  ...
  assert(viewerSource.includes(".short-video-control-volume::before") && viewerSource.includes("bottom: 30px;") && viewerSource.includes("bottom: -12px;"), "desktop volume controls must provide a continuous hover bridge between the mute button and vertical slider");
