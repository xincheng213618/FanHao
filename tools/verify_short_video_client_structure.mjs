...
function verifyWebDedicatedEntry() {
  ...
  assert(playerSource.split(/\r?\n/).length <= 6520, "the Web short-video composition root must stay below 6520 lines");
  ...
  assert(shortVideoRuntimeSource.includes('SHORT_VIDEO_LIST_CACHE_SCHEMA = "aggregate-search-v2-author-deleted"') && shortVideoRuntimeSource.includes("`${SHORT_VIDEO_LIST_CACHE_SCHEMA}:${url.pathname}?${normalized.toString()}`"), "short-video list response schema changes must rotate the persistent cache namespace");
