...
assert(androidIndexHtmlSource.includes("./styles.css?v=20260812-collection-management-01") && androidRootStylesSource.includes("./modules/short-videos/styles.css?v=20260812-collection-management-01") && androidShortVideoStylesSource.includes("./styles/list.css?v=20260812-collection-management-01"), "Android collection management styles must propagate through the stylesheet cache-version chain");
...
assert(androidNativeFeedTransportSource.includes("new InputStreamReader(connection.getInputStream(), StandardCharsets.UTF_8)") && !androidNativeFeedTransportSource.includes("new String(buffer"), "Android native feed transport must preserve UTF-8 characters split across network chunks");
...
assert(appSource.includes("function discardPushedView()") && appSource.includes("returnToStackView({ discardHistoryEntry: true })") && appSource.includes("options.discardHistoryEntry") && appSource.includes("window.history.back()") && androidIndexHtmlSource.includes("./app.js?v=20260812-collection-management-03"), "Android collection deletion must discard both the in-memory stack entry and its browser-history entry, with the app cache version advanced");
assert(
