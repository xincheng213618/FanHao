...
import { retryFavoriteFolderRequest } from "../android-client/www/modules/fanhao/features/works/favorite-folders.js";
...
const androidDom = read("android-client/www/js/dom.js");
...
const androidFavoriteFolders = read("android-client/www/modules/fanhao/features/works/favorite-folders.js");
const androidFavoritePage = read("android-client/www/modules/fanhao/features/works/favorite-page.js");
...
assert(androidFanhaoChrome.includes("if (active)") && androidFanhaoChrome.includes("openSortDialog(host, activeSort)"), "tapping the active FanHao chrome tag must open its sort sheet");
...
assert(androidWorkViews.includes("function activeWorkListState()") && androidWorkViews.includes("getWorkListRequestState: () => ({ filter: activeWorkListState().getServerFilterMode(), sort: activeWorkListState().getServerSortMode() })"), "Android ordinary and favorite work routes must each expose one authoritative server filter and sort snapshot");
...
assert(androidWorkViews.includes('return workCollectionPath("history", {') && androidFavoritePage.includes('workCollectionPath("favorites", {') && androidWorkViews.includes('return favoriteViews.render(params, isActive)') && androidWorkViews.includes('const path = `/api/works?'), "Android favorites must remain a works-filter route while using exactly one collection request source per render");
let favoriteFolderAttempts = 0;
const favoriteFolderRetryResult = await retryFavoriteFolderRequest(async () => {
  favoriteFolderAttempts += 1;
  if (favoriteFolderAttempts === 1) throw Object.assign(new Error("fixture busy"), { status: 503, retryable: true });
  return "ok";
}, { delaysMs: [0], sleep: async () => {} });
assert.equal(favoriteFolderRetryResult, "ok", "Android favorite folder requests must recover from one explicitly retryable busy response");
assert.equal(favoriteFolderAttempts, 2, "Android favorite folder retries must be finite");
await assert.rejects(() => retryFavoriteFolderRequest(async () => {
  throw Object.assign(new Error("ordinary failure"), { status: 503, retryable: false });
}, { delaysMs: [0], sleep: async () => {} }), /ordinary failure/u, "Android favorite folder requests must not retry an ordinary 503");
assert(androidFavoriteFolders.includes("let activeBaseUrl") && androidFavoriteFolders.includes("mutationRevision === requestRevision") && androidFavoriteFolders.includes('throw new Error("服务器已切换，请重新操作")'), "Android favorite folder state must isolate servers and reject stale list or mutation responses");
assert(androidFavoriteFolders.includes("const workMutationTails = new Map()") && androidFavoriteFolders.includes("function enqueueWorkMutation") && androidFavoriteFolders.includes("previous.then"), "Android favorite changes on one work must serialize behind one mutation tail");
assert(androidFavoriteFolders.includes("refreshRequestedRevision") && androidFavoriteFolders.includes("failedRefreshRevision") && androidFavoriteFolders.includes("scheduleFolderRefresh()"), "Android favorite mutations must retain coalesced refresh demand across stale or failed requests");
assert(!androidWorkActions.includes("work.favorite = previous") && androidApp.includes("library.user = { ...(library.user || {}), ...user }") && androidApp.includes('Object.hasOwn(user, "favoriteCount")'), "Android favorite state must avoid outer rollback and preserve unrelated user fields during partial count updates");
assert(androidFavoriteFolders.includes('panel.setAttribute("role", "dialog")') && androidFavoriteFolders.includes('input.focus({ preventScroll: true })') && androidFavoriteFolders.includes('document.addEventListener("keydown", handleDocumentKeydown, true)') && androidFavoriteFolders.includes('panel.setAttribute("aria-busy"'), "Android favorite folder sheets must retain keyboard capture and enabled focus while pending");
...
assert(androidFanhaoChrome.includes('{ key: "favorites", label: "收藏", view: "works", params: { favorite: "1" } }') && androidFanhaoChrome.includes('host.navigation.showView(tab.view, tab.params || {}, { resetStack: true })') && androidFanhaoChrome.includes('String(params.favorite || "") === "1"'), "Android FanHao chrome must expose favorites through the canonical works-filter route");
...
assert(androidIndexHtml.includes("styles.css?v=20260812-android-work-move-02") && androidStyles.includes("css/base.css?v=20260730-mobile-search-ui-41") && androidStyles.includes("css/sections.css?v=20260812-android-work-move-02") && androidStyles.includes("css/lists.css?v=20260730-fanhao-cover-ui-43") && androidStyles.includes("modules/fanhao/styles.css?v=20260811-favorite-folders-02") && androidStyles.includes("modules/tools/styles.css?v=20260818-vision-exploration-01") && androidStyles.includes("modules/novels/styles.css?v=20260731-novel-actions-ui-53") && androidStyles.includes("modules/music/home.css?v=20260730-music-palette-ui-42") && androidStyles.includes("modules/short-videos/styles.css?v=20260812-collection-management-01"), "Android work-move styles must refresh through the full application style chain without dropping adjacent modules");
...
assert(androidClientVersion && androidIndexHtml.includes(`app.js?v=${androidClientVersion}`) && androidApp.includes(`config.js?v=${androidClientVersion}`) && androidApp.includes(`cache.js?v=${androidClientVersion}`) && androidApp.includes("dom.js?v=20260812-android-work-move-02") && androidCacheSource.includes(`config.js?v=${androidClientVersion}`), "Android behavior must refresh the complete application and response-cache identity chain");
assert(androidToolModule.includes('tool-views.js?v=20260818-vision-exploration-01'), "Android tools dashboard must use a fresh view module URL");
...
assert(androidApp.includes("function returnToStackView(options = {})") && androidApp.includes("replaceHistory: true") && androidFanhaoModule.includes("host.navigation.goBack()"), "FanHao search close must replace its route with the originating in-app view while allowing explicit pushed-history discard");
...
assert(androidFanhaoIndex.includes('detail-views.js?v=20260812-android-work-move-02') && androidDetailViews.includes('actions.js?v=20260812-android-work-move-02') && androidDetailViews.includes('work-move.js?v=20260812-android-work-move-02') && androidDetailViews.includes('android-player.js?v=20260721-fanhao-media-relocate-16') && androidDetailViews.includes('detail-toolbar.js?v=20260730-fanhao-work-detail-ui-46'), "Android work-move actions must refresh without dropping media recovery or the contextual toolbar");
assert(androidWorkViews.includes('page-data-service.js?v=20260811-favorite-folders-02') && androidWorkViews.includes('favorite-page.js?v=20260811-favorite-folders-02') && androidWorkViews.includes('work-data-signature.js?v=20260811-favorite-folders-02') && androidFavoritePage.includes('favorite-folders.js?v=20260811-favorite-folders-02') && androidWorkViews.includes('detail-data-service.js?v=20260717-fanhao-touch-intent-01'), "Android favorite-folder and page-race services must retain fresh module URLs");
assert(androidFanhaoIndex.includes('work-views.js?v=20260811-favorite-folders-02') && androidWorkViews.includes('cache.js?v=20260721-fanhao-actor-counts-17') && androidWorkViews.includes('work-filtering.js?v=20260726-work-sort-01') && androidWorkViews.includes('search-page.js?v=20260721-fanhao-search-suggestions-19'), "Android favorite folders must refresh work views without dropping search suggestions, actor-count caching, or dense browsing");
...
assert(androidFanhaoModule.includes('chrome.js?v=20260812-android-work-move-02') && androidFanhaoModule.includes('index.js?v=20260812-android-work-move-02') && androidFanhaoModule.includes('prefix-views.js?v=20260730-fanhao-nav-ui-44'), "Android work moves must refresh the FanHao view entry without dropping ranking or brand browsing");
