...
try {
  ...
  assert.match(androidMusicStyles, /body\.music-mobile-view\s*\{[\s\S]*?--music-accent: #151718;[\s\S]*?--music-accent-text: #151718;[\s\S]*?--music-playing-accent: #00a968;[\s\S]*?--bg: #f1f5f7;[\s\S]*?--field: #e2e7ec;[\s\S]*?--ink: #000000;[\s\S]*?--muted: #6d7071;/, "Android music should use a graphite interaction palette and reserve restrained green for live playback while keeping the measured canvas, field, and contrast-safe text");
  ...
  assert.match(androidMusicStyles, /body\.music-mobile-view \.music-mobile-library-head\.is-home \.music-mobile-play-all\s*\{[\s\S]*?background: var\(--ink\);[\s\S]*?color: var\(--bg\);/, "Android music home should keep the large continue action neutral instead of adding another fluorescent green surface");
  ...
  assert.match(androidMusicViewSource, /DEFAULT_COVER_URL = "\.\/assets\/music\/default-cover-v2-neutral\.jpg\?v=20260730-music-home-ui-40"/, "Android music must use a real neutral bundled fallback artwork instead of text initials or green-heavy placeholder art when embedded covers are missing");
  const neutralDefaultCoverPath = path.join(root, "android-client", "www", "assets", "music", "default-cover-v2-neutral.jpg");
  assert(fs.existsSync(neutralDefaultCoverPath), "Android neutral fallback artwork must be bundled");
  assert(fs.statSync(neutralDefaultCoverPath).size < 500_000, "Android neutral fallback artwork should stay compact enough for the bundled client");
  ...
  assert.match(androidMusicViewSource, /symbol = document\.createElement\("img"\)[\s\S]*?symbol\.src = `\.\/assets\/icons\/music-\$\{name\}\.svg\?v=20260730-music-home-ui-40`/, "Android music controls should render bundled icon-library assets instead of text glyphs or CSS drawings");
