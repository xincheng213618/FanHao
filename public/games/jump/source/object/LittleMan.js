...
class LittleMan {
  ...
  initEventListener () {
    ...
    const mousedownName = supportsPointer ? 'pointerdown' : isMobile ? 'touchstart' : 'mousedown';
    const mouseupName = supportsPointer ? 'pointerup' : isMobile ? 'touchend' : 'mouseup';
    ...
    // 监听松开事件
    window.addEventListener(mouseupName, (event) => {
      event.preventDefault();
  ...
  afterJump(state) {
    ...
    if (state === LittleMan.STATE.nextBox) {
      ...
      // 通知资料库集成层更新当前分数。
      this.successCount += 1;
      window.dispatchEvent(new CustomEvent('fanhao:jump-score', {
        detail: {score: this.successCount}
      }));
    ...
    if (state === LittleMan.STATE.outRange ||
        ...
        state === LittleMan.STATE.next_edge_front ||
        state === LittleMan.STATE.next_edge_back) {
      window.dispatchEvent(new CustomEvent('fanhao:jump-game-over', {
        detail: {score: this.successCount}
      }));
    }
  }
