...
export function createShortVideoDeleteActions(options = {}) {
  ...
  async function deleteVideo(video, requestOptions = {}) {
    ...
    const result = await requestShortVideoDelete(
      api,
  ...
  async function deleteSelected(ids = []) {
    ...
    if (!confirmDelete(`确定删除选中的 ${selected.length} 条短视频吗？\n\n会删除资料库记录以及这些记录引用且未被其他记录引用的本地文件。`)) return null;
    const result = await requestShortVideoDelete(api, "/api/short-videos", {
      method: "DELETE",
      body: { ids: selected }
