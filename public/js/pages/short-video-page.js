export function createShortVideoPage(deps) {
  ...
  let shortVideoSearchSuggestTimer = 0;
  let shortVideoSearchSuggestRequestId = 0;
  ...
  const SHORT_VIDEO_SEARCH_HISTORY_KEY = "fanhao.shortVideo.searchHistory";
  ...
  function renderShortVideoDiscovery() {
    ...
    form.className = "short-video-search";
    ...
    input.setAttribute("aria-label", "搜索短视频、图文、作者或话题");
    input.setAttribute("aria-autocomplete", "list");
    input.setAttribute("aria-controls", "short-video-search-suggestions");
    input.setAttribute("aria-expanded", "false");
    const clear = document.createElement("button");
    clear.type = "button";
    clear.className = "short-video-search-clear";
    clear.setAttribute("aria-label", "清除短视频搜索");
    ...
    panel.id = "short-video-search-suggestions";
    ...
    const choose = (item) => {
      const query = String(item?.query || item?.label || "").replace(/^#/, "").trim();
      input.value = query;
      clear.hidden = !query;
      closePanel();
    ...
    const renderSuggestions = (items, label = "搜索建议") => {
      ...
      if (label === "最近搜索") {
        ...
        removeHistory.textContent = "清空";
        removeHistory.setAttribute("aria-label", "清空短视频搜索历史");
        removeHistory.addEventListener("pointerdown", (event) => event.preventDefault());
        removeHistory.addEventListener("click", () => {
          clearShortVideoSearchHistory();
          closePanel();
        });
        heading.append(removeHistory);
      }
      panel.append(heading);
      suggestionItems.forEach((item, index) => {
        const button = document.createElement("button");
        button.type = "button";
        button.id = `short-video-search-option-${index}`;
    ...
    const showRecent = () => {
      const recent = readShortVideoSearchHistory().map((query) => ({ kind: "recent", label: query, query, detail: "最近搜索" }));
      renderSuggestions(recent, "最近搜索");
    };
    const requestSuggestions = (value) => {
      window.clearTimeout(shortVideoSearchSuggestTimer);
      const query = String(value || "").trim();
      if (!query) {
        showRecent();
        return;
      }
      shortVideoSearchSuggestTimer = window.setTimeout(async () => {
        const requestId = ++shortVideoSearchSuggestRequestId;
        try {
          const params = new URLSearchParams({ q: query, limit: "8" });
          if (state.shortVideo.media && state.shortVideo.media !== "all") params.set("media", state.shortVideo.media);
          const result = await api(`/api/short-videos/suggestions?${params}`);
          if (requestId !== shortVideoSearchSuggestRequestId || !panel.isConnected || input.value.trim() !== query) return;
          renderSuggestions(result.suggestions || []);
        } catch {
          if (requestId === shortVideoSearchSuggestRequestId) closePanel();
        }
      }, 220);
    };
    const moveActive = (direction) => {
      if (panel.hidden || !suggestionItems.length) return false;
    ...
    form.addEventListener("submit", (event) => {
      event.preventDefault();
      closePanel();
      commitShortVideoSearch(input.value);
    });
    ...
    clear.addEventListener("click", () => {
      input.value = "";
      clear.hidden = true;
      if (state.shortVideo.query) commitShortVideoSearch("");
      else {
        input.focus();
        showRecent();
      }
    ...
    form.append(field, submit, panel);
  ...
  function commitShortVideoSearch(value) {
    ...
    if (query) writeShortVideoSearchHistory(query);
  ...
  function readShortVideoSearchHistory() {
    try {
      const values = JSON.parse(window.localStorage.getItem(SHORT_VIDEO_SEARCH_HISTORY_KEY) || "[]");
      return Array.isArray(values) ? values.map((item) => String(item || "").trim()).filter(Boolean).slice(0, 6) : [];
    } catch {
      return [];
    }
  }

  function writeShortVideoSearchHistory(query) {
    try {
      const next = [query, ...readShortVideoSearchHistory().filter((item) => item !== query)].slice(0, 6);
      window.localStorage.setItem(SHORT_VIDEO_SEARCH_HISTORY_KEY, JSON.stringify(next));
    } catch {}
  }

  function clearShortVideoSearchHistory() {
    try {
      window.localStorage.removeItem(SHORT_VIDEO_SEARCH_HISTORY_KEY);
    } catch {}
  }
  ...
  function renderBrowser() {
    ...
    searchText.textContent = state.shortVideo.query
      || (state.shortVideo.topic ? `#${state.shortVideo.topic}` : "")
      || (state.shortVideo.sound ? `♫ ${state.shortVideo.soundInfo?.title || "原声流"}` : "")
      || "搜索你感兴趣的内容";
  ...
  function syncShortVideoBrowserSearchDom() {
    const searchText = els.workGrid?.querySelector?.(".short-video-browser-search-text");
