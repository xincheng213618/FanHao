...
export const ADMIN_SCRIPT_DEFINITIONS = [
  ...
  },
  {
    ...
    risk: "careful",
    description: "后台重建套图、电影、电视剧和动漫索引；欧美内容统一由番号核心库维护。",
    ...
    fields: [
      {
        ...
        options: [
          ...
          { value: "media", label: "只扫电影/电视剧/动漫" },
          { value: "movie", label: "只扫电影" },
          { value: "tv", label: "只扫电视剧" },
          { value: "anime", label: "只扫动漫" }
        ]
      }
    ]
  },
  {
    id: "tuimzz-photo-sync",
    title: "检查微密圈套图更新",
    category: "图库",
    runtime: "python",
    script: path.join("tools", "sync_tuimzz_photo_sets.py"),
    risk: "careful",
    description: "对照 tuimzz 微密圈分类与 T:\\微密圈，生成更新、本地缺失和百度云链接报告；不会下载或解压。",
    refreshHints: [],
    invalidates: [],
    fields: [
      {
        ...
        type: "text",
        flag: "--cookie-file",
        default: path.join("T:\\微密圈", "_catalog", "tuimzz-cookie.txt"),
        placeholder: "T:\\微密圈\\_catalog\\tuimzz-cookie.txt",
        help: "只传本机文件路径；Cookie 内容不会写入命令、报告或日志。"
      },
      { name: "resolveLinks", label: "解析需要更新项的百度云链接", type: "checkbox", flag: "--resolve-links", default: true },
      { name: "resolveAll", label: "刷新全部百度云链接", type: "checkbox", flag: "--resolve-all", default: false },
      { name: "maxPages", label: "最多分类页", type: "number", flag: "--max-pages", default: 20, min: 1, max: 100, step: 1 },
      { name: "delay", label: "请求间隔秒", type: "number", flag: "--delay", default: 0.2, min: 0, max: 30, step: 0.1 }
    ]
  },
  {
    id: "tuimzz-photo-import",
    title: "导入微密圈下载",
    category: "图库",
    runtime: "python",
    script: path.join("tools", "import_tuimzz_downloads.py"),
    risk: "write",
    description: "把明确编号的 7z 下载安全转换成无密码 RAR 并写入 T:\\微密圈；默认只预览且永不删除源文件。",
    refreshHints: [],
    invalidates: [],
    fields: [
      { name: "inputRoot", label: "下载根目录", type: "text", flag: "--input-root", default: "D:\\" },
      {
        name: "roots",
        label: "编号目录（每行一个）",
        type: "textarea-list",
        ...
        default: "",
        placeholder: "285\n389A",
        help: "必须明确列出；不会自动扫描整个 D 盘。"
      },
      { name: "execute", label: "实际转换并写入 T 盘", type: "checkbox", flag: "--execute", default: false },
      { name: "workers", label: "并发数", type: "number", flag: "--workers", default: 2, min: 1, max: 8, step: 1 },
      { name: "maxItems", label: "最多处理包数（0=不限）", type: "number", flag: "--max-items", default: 0, min: 0, step: 1 }
    ]
  },
  {
    id: "core-local-scan",
  ...
  },
  {
    ...
    fields: [
      { name: "write", label: "写入封面缓存", type: "checkbox", flag: "--write", default: true },
      {
        ...
        default: "all",
        options: [
          { value: "all", label: "全部资料库" },
          { value: "western", label: "只处理欧美 R 盘" }
        ]
      },
      { name: "limit", label: "生成数量", type: "number", flag: "--limit", default: 20, min: 0, max: 10000, step: 1 },
      { name: "concurrency", label: "并发数", type: "number", flag: "--concurrency", default: 3, min: 1, max: 8, step: 1 },
      { name: "overwrite", label: "覆盖已有封面", type: "checkbox", flag: "--overwrite", default: false },
      { name: "retryErrors", label: "重试失败文件", type: "checkbox", flag: "--retry-errors", default: false },
