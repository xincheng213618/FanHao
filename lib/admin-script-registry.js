...
export const ADMIN_SCRIPT_DEFINITIONS = [
  ...
  },
  {
    ...
    risk: "careful",
    description: "后台重建套图、欧美、电影和电视剧索引；只更新 image-library-index.json，不清理番号缓存。",
    ...
    fields: [
      {
        ...
        options: [
          ...
          { value: "media", label: "只扫欧美/电影/电视剧" },
          { value: "movie", label: "只扫电影" },
          { value: "tv", label: "只扫电视剧" },
          { value: "western", label: "只扫欧美" }
        ]
      }
    ]
  },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: "limit", label: "生成数量", type: "number", flag: "--limit", default: 50, min: 0, max: 10000, step: 1 },
      { name: "concurrency", label: "并发数", type: "number", flag: "--concurrency", default: 2, min: 1, max: 16, step: 1 }
    ]
  },
  {
    id: "sync-sidecars",
