...
- Python 3.10+ is required; keep `pyproject.toml`, launchers, and documentation aligned
...
### FanHao Ownership
- This directory is the authoritative downloader implementation used by FanHao's short-video download manager.
- Do not add runtime dependencies on a sibling checkout such as `Desktop\\Tool\\douyin-downloader`.
- Upstream comparisons are maintenance aids only; changes required by FanHao are made and tested here.
- Preserve the upstream MIT license and third-party source headers when updating imported code.

### Testing Requirements
- Run: `python -m pytest tests/`
- Async tests use `pytest-asyncio` with `asyncio_mode = "auto"`
- Linting: `ruff check .` (target Python 3.10, line-length 100)
