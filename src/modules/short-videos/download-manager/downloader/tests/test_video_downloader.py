...
@pytest.mark.asyncio
async def test_download_aweme_assets_gallery_downloads_and_indexes_background_music(tmp_path, monkeypatch):
    downloader, api_client = _build_downloader(tmp_path)
    downloader.config.update(
        music=False,
        gallery_music=True,
    ...
    aweme_id = "7660744527430769851"
    aweme_data = {
        "aweme_id": aweme_id,
        "desc": "图集背景音乐",
        "image_post_info": {
            "images": [
                {"display_image": {"url_list": ["https://example.com/gallery.jpg"]}}
            ]
        },
        "music": {
            "id_str": "7657899274080209673",
            "title": "测试原声",
            "author": "测试作者",
            "play_url": {"url_list": ["https://example.com/background.mp3"]},
        },
    }
    ...
    assert success is True
    assert any(path.name.endswith("_music.mp3") for path in saved_paths)
    manifest_path = tmp_path / "download_manifest.jsonl"
    record = json.loads(manifest_path.read_text(encoding="utf-8").strip().splitlines()[-1])
    assert record["music"]["id_str"] == "7657899274080209673"
    assert record["music"]["title"] == "测试原声"
    assert record["music"]["local_file_path"].endswith("_music.mp3")
    assert any(path.endswith("_music.mp3") for path in record["file_paths"])
    assert record["fanhaoMedia"] == {
        "type": "gallery",
        "galleryCount": 1,
        "galleryItems": ["image"],
    }
