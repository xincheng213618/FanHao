...
class MsTokenManager:
    ...
    _token_cache_ttl_seconds: int = 1800
    _lock = RLock()
    ...
    def ensure_ms_token(self, cookies: Dict[str, str]) -> str:
        ...
        if current:
            with self._lock:
                self.__class__._cached_token = current
                self.__class__._cached_token_at = time.time()
            return current

        now = time.time()
        with self._lock:
            if (
                self.__class__._cached_token
                and (now - self.__class__._cached_token_at) < self.__class__._token_cache_ttl_seconds
            ):
                return self.__class__._cached_token

            real = self.gen_real_ms_token()
            token = real or self.gen_false_ms_token()
            self.__class__._cached_token = token
            self.__class__._cached_token_at = time.time()
            return token
