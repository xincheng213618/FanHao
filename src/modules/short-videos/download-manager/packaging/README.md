...
1. 用 PyInstaller 构建无控制台的管理器 EXE，并打入桌面 WebView、静态页面、采集脚本、登录助手和 `playwright-core`。
...
5. 在随机隔离端口以无窗口测试模式启动组装后的管理器，检查 `/api/state`、已下载页面、登录助手和下载器命令。
...
安装后会打开独立桌面窗口，关闭窗口即结束后台服务；“已下载”页直接读取并播放下载目录中的本地作品。数据库和日志位于 `%LOCALAPPDATA%\DouyinDownloadManager`。构建过程不会复制开发电脑上的 Cookie、数据库、日志或视频。
