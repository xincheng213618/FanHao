...
| `GET` | `/api/short-videos/collections/:collectionId/videos?limit=&offset=` | Videos ordered by newest membership first |
...
The API accepts a canonical catalog id or an existing aweme id for video membership, but stores and returns the canonical video id. Repeating an add or remove succeeds without changing the original add time. Deleting a catalog video removes its memberships; deleting a collection removes its membership rows but never deletes video records or files.
