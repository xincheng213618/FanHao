...
export function createMusicViews(deps) {
  ...
  function applyRouteParams(params = {}) {
    ...
    state.favorite = Boolean(params.favorite || params.fav);
  ...
  function renderShell() {
    ...
    shell.append(renderLibraryHeader(), renderTabs(), renderControls(), renderTrackList(), renderMiniPlayer());
  ...
  function renderControls() {
    ...
    search.disabled = state.mode !== "library";
    const applySearch = () => updateListParams({ query: search.value.trim(), mode: "library", favorite: state.favorite });
    ...
    searchButton.disabled = state.mode !== "library";
  ...
  function musicMeta(data = state.data) {
    ...
    if (state.favorite) return `${formatNumber(total || 0)} 首 · 收藏`;
    if (totals.tracks) {
...
function normalizeMode(value) {
  ...
  return ["library", "history"].includes(mode) ? mode : DEFAULT_MODE;
}
